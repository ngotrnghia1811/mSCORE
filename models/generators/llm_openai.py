...
class OpenAI(Generator):
    ...
    def openai_api_calculate_cost(self,usage):
        pricing = {
            ...
            'o1-mini': {
                'prompt': 0.003, # US$3.00 / 1M tokens
                'completion': 0.012, # US$12.00 / 1M tokens
            },
            'o1-preview': {
                'prompt': 0.015, # US$15.00 / 1M tokens
                'completion': 0.06, # US$60.00 / 1M tokens
            }
        }
    ...
    def compile_prompt(self, input, task_name):
        """
        openai chat template
        """
        
        if task_name == 'mscqa_gen':
            return self._mscqa_gen_prompt(input)
        elif task_name == 'culbank_gen':
            return self._culbank_gen_prompt(input)
        elif task_name == 'expand':
            return self._expand_prompt(input)
        elif task_name == 'implicit':
            return self._implicit_prompt(input)
        elif task_name == 'inference':
            return self._inference_prompt(input)
    

    def _mscqa_gen_prompt(self, inp):
        """
        prompt for converting a QA in mCSQA into skill_format
        """
        input_prompt = f"""
        INPUT:
        {inp}
        ...
        messages = [
            {"role": "system", "content": mcsqa_gen},
            {"role": "user", "content": input_prompt}
        ]

        return messages
    
    def _culbank_gen_prompt(self, inp):
        """
        prompt for converting a situation in CulBank into skill_format
        """
    ...
    def _inference_prompt(self, inp):
